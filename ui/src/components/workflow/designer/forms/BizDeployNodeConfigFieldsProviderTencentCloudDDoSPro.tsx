...
const BizDeployNodeConfigFieldsProviderTencentCloudDDoSPro = () => {
  ...
  const formInst = Form.useFormInstance<typeof formSchema>();
  ...
  return (
    <>
      <Form.Item
        ...
        label={t("workflow_node.deploy.form.tencentcloud_ddospro_endpoint.label")}
        rules={[formRule]}
        tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.tencentcloud_ddospro_endpoint.tooltip") }}></span>}
      >
        <Input allowClear placeholder={t("workflow_node.deploy.form.tencentcloud_ddospro_endpoint.placeholder")} />
      </Form.Item>

      <Form.Item
        name={[parentNamePath, "instanceId"]}
        initialValue={initialValues.instanceId}
        label={t("workflow_node.deploy.form.tencentcloud_ddospro_instance_id.label")}
        rules={[formRule]}
        tooltip={<span dangerouslySetInnerHTML={{ __html: t("workflow_node.deploy.form.tencentcloud_ddospro_instance_id.tooltip") }}></span>}
      >
        <Input placeholder={t("workflow_node.deploy.form.tencentcloud_ddospro_instance_id.placeholder")} />
      </Form.Item>
      ...
      <Show when={fieldDomainMatchPattern !== DOMAIN_MATCH_PATTERN_CERTSAN}>
        <Form.Item
          ...
          label={t("workflow_node.deploy.form.tencentcloud_ddospro_domain.label")}
          rules={[formRule]}
        >
          <Input placeholder={t("workflow_node.deploy.form.tencentcloud_ddospro_domain.placeholder")} />
        </Form.Item>
      </Show>
    </>
  );
...
const _default = Object.assign(BizDeployNodeConfigFieldsProviderTencentCloudDDoSPro, {
  getInitialValues,
  getSchema,
});
